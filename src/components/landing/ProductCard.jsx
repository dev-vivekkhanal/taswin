...
const ProductCard = ({ title, image, image2, description, variants }) => {
  ...
  return (
    <div className="w-fit flex flex-col items-center cursor-pointer">
      <div className={`w-full max-w-[250px] transition-all duration-300 ease-out relative rounded-[21px] shadow-md`} onMouseEnter={() => setHoverDesc(true)} onMouseLeave={() => setHoverDesc(false)}>
        {
          image && image2 ?
            // <img src={hoverDesc ? image2 : image} className="w-full rounded-[21px]" alt="" />
      ...
      <div>
        ...
        <div className={`w-full overflow-hidden transition-all duration-300 ease-out ${hoverDesc && description ? 'h-[100px] overflow-hidden' : 'h-0'}`}>
          <p className='text-[12px] text-center px-2 py-3 mt-2'>{description}</p>
        </div>
        <div className={`h-1 w-full max-w-[230px] mx-auto ${hoverDesc ? 'bg-secondary bg-opacity-70' : 'bg-secondary'} ${hoverDesc && description ? 'mt-2' : 'mt-8'} `}></div>
      </div>
    </div>
  )
}
